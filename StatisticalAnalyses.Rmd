...
```{r}
etaSquared(aov(ClassificationAccuracy ~ AttendIgnore*Hemisphere*Lobe+Error(Subject/(AttendIgnore*Hemisphere*Lobe)), data=data), type = 2, anova = FALSE )
```
...
interaction.plot(lobe2LH$AttendIgnore,lobe2LH$RegionID,lobe2LH$ClassificationAccuracy)

```


# Compute regressions between attended and ignored residual correlations (also do the same for classification accuracy) 
```{r}
data <- read.csv("data_residual_correlations_avgOverSubjects.csv")
...
data <- read.csv("data_classification_accuracy_avgOverSubjects.csv")
...
```{r}
# load data
data.RC <- read.csv("data_residual_correlations_avgOverSubjects.csv")
data.CA <- read.csv("data_classification_accuracy_avgOverSubjects.csv")
data.both <- data.CA
...
model1<-lm(data.both.LH$ClassificationAccuracy~data.both.LH$ResidualCorrelations)
model2<-lm(data.both.LH$ClassificationAccuracy~data.both.LH$ResidualCorrelations+data.both.LH$AttendIgnore)
model3<-lm(data.both.LH$ClassificationAccuracy~data.both.LH$ResidualCorrelations+data.both.LH$AttendIgnore+data.both.LH$ResidualCorrelations*data.both.LH$AttendIgnore)
anova(model1,model2,test = "Chisq")
anova(model2,model3,test = "Chisq")


model1<-lm(data.both.RH$ClassificationAccuracy~data.both.RH$ResidualCorrelations)
model2<-lm(data.both.RH$ClassificationAccuracy~data.both.RH$ResidualCorrelations+data.both.RH$AttendIgnore)
model3<-lm(data.both.RH$ClassificationAccuracy~data.both.RH$ResidualCorrelations+data.both.RH$AttendIgnore+data.both.RH$ResidualCorrelations*data.both.RH$AttendIgnore)
anova(model1,model2,test = "Chisq")
anova(model2,model3,test = "Chisq")
```
...
# ANOVA by stimulus condition (residuals)
```{r}
data <- read.csv("data_residual_correlations_byStimulusCondition.csv")
data$Subject = as.factor(rep(c(1:21),2736))
data$Hemisphere = as.factor(data$Hemisphere)
data$Lobe = as.factor(data$Lobe)
data$AttendIgnore = as.factor(data$AttendIgnore)
data$RegionID = as.factor(data$RegionID)
...
data_avgoverhem <- data[data$Hemisphere==1,c("ResidualCorrelations","AttendIgnore","Lobe","RegionID","StimCond","Subject")]
data_avgoverhem$ResidualCorrelations<-rowMeans(data.frame(data[data$Hemisphere==1,1],data[data$Hemisphere==2,1]),na.rm=TRUE)

summary(aov(ResidualCorrelations ~ Lobe+AttendIgnore+Lobe:AttendIgnore+Error(Subject/(Lobe*AttendIgnore)), data=data_avgoverhem[data_avgoverhem$StimCond==1,]))
summary(aov(ResidualCorrelations ~ Lobe+AttendIgnore+Lobe:AttendIgnore+Error(Subject/(Lobe*AttendIgnore)), data=data_avgoverhem[data_avgoverhem$StimCond==2,]))
summary(aov(ResidualCorrelations ~ Lobe+AttendIgnore+Lobe:AttendIgnore+Error(Subject/(Lobe*AttendIgnore)), data=data_avgoverhem[data_avgoverhem$StimCond==3,]))
summary(aov(ResidualCorrelations ~ Lobe+AttendIgnore+Lobe:AttendIgnore+Error(Subject/(Lobe*AttendIgnore)), data=data_avgoverhem[data_avgoverhem$StimCond==4,]))

summary(aov(ResidualCorrelations ~ Lobe*AttendIgnore*StimCond+Error(Subject/(Lobe*AttendIgnore*StimCond)), data=data_avgoverhem[complete.cases(data_avgoverhem),]))
```


# ANOVA by stimulus condition (classification)
```{r}
data <- read.csv("data_classification_accuracy_byStimulusCondition.csv")
data$Subject = as.factor(rep(c(1:21),2736))
data$Hemisphere = as.factor(data$Hemisphere)
data$Lobe = as.factor(data$Lobe)
data$AttendIgnore = as.factor(data$AttendIgnore)
...
data_avgoverhem <- data[data$Hemisphere==1,c("ClassificationAccuracy","AttendIgnore","Lobe","RegionID","StimCond","Subject")]
data_avgoverhem$ClassificationAccuracy<-rowMeans(data.frame(data[data$Hemisphere==1,1],data[data$Hemisphere==2,1]))

summary(aov(ClassificationAccuracy ~ Lobe+AttendIgnore+Lobe:AttendIgnore+Error(Subject/(Lobe*AttendIgnore)), data=data_avgoverhem[data_avgoverhem$StimCond==1,]))
summary(aov(ClassificationAccuracy ~ Lobe+AttendIgnore+Lobe:AttendIgnore+Error(Subject/(Lobe*AttendIgnore)), data=data_avgoverhem[data_avgoverhem$StimCond==2,]))
summary(aov(ClassificationAccuracy ~ Lobe+AttendIgnore+Lobe:AttendIgnore+Error(Subject/(Lobe*AttendIgnore)), data=data_avgoverhem[data_avgoverhem$StimCond==3,]))
summary(aov(ClassificationAccuracy ~ Lobe+AttendIgnore+Lobe:AttendIgnore+Error(Subject/(Lobe*AttendIgnore)), data=data_avgoverhem[data_avgoverhem$StimCond==4,]))

summary(aov(ClassificationAccuracy ~ Lobe+AttendIgnore+StimCond+Lobe:AttendIgnore+Lobe:StimCond+AttendIgnore:StimCond+AttendIgnore:Lobe:StimCond+Error(Subject/(Lobe*AttendIgnore*StimCond)), data=data_avgoverhem))
```
