...
% Load csvs with final data for each Glasser ROI for Classification and
% Residual Correlations
% Columns here are: 1) Data (Residual Correlation or Classification
% Accuracy), 2) Hemisphere, 3) AttendIgnore, 4) Lobe
class_results = xlsread('/Users/ariellekeller/Google Drive/SelecAttnProj/ANOVAs_Glasser/data_ANOVA_Glasser_Class_excludeVTC.xls');
resid_results = xlsread('/Users/ariellekeller/Google Drive/SelecAttnProj/ANOVAs_Glasser/data_ANOVA_Glasser_Resid_excludeVTC.xls');

class_results_avg = [];
resid_results_avg = [];

for takeAvg = 1:688
    class_results_avg = [class_results_avg; mean(class_results(((12*takeAvg)-11):(12*takeAvg),1)) class_results(((12*takeAvg)-11),2) class_results(((12*takeAvg)-11),3) class_results(((12*takeAvg)-11),4)];
    resid_results_avg = [resid_results_avg; mean(resid_results(((12*takeAvg)-11):(12*takeAvg),1)) resid_results(((12*takeAvg)-11),2) resid_results(((12*takeAvg)-11),3) resid_results(((12*takeAvg)-11),4)];
end
...
Frontal_Att_LH = corrcoef(class_Frontal_LH(class_Frontal_LH(:,3)==1,1),resid_Frontal_LH(resid_Frontal_LH(:,3)==1,1));
Frontal_Ig_LH = corrcoef(class_Frontal_LH(class_Frontal_LH(:,3)==2,1),resid_Frontal_LH(resid_Frontal_LH(:,3)==2,1));
Frontal_Att_RH = corrcoef(class_Frontal_RH(class_Frontal_RH(:,3)==1,1),resid_Frontal_RH(resid_Frontal_RH(:,3)==1,1));
Frontal_Ig_RH = corrcoef(class_Frontal_RH(class_Frontal_RH(:,3)==2,1),resid_Frontal_RH(resid_Frontal_RH(:,3)==2,1));

Parietal_Att_LH = corrcoef(class_Parietal_LH(class_Parietal_LH(:,3)==1,1),resid_Parietal_LH(resid_Parietal_LH(:,3)==1,1));
Parietal_Ig_LH = corrcoef(class_Parietal_LH(class_Parietal_LH(:,3)==2,1),resid_Parietal_LH(resid_Parietal_LH(:,3)==2,1));
Parietal_Att_RH = corrcoef(class_Parietal_RH(class_Parietal_RH(:,3)==1,1),resid_Parietal_RH(resid_Parietal_RH(:,3)==1,1));
Parietal_Ig_RH = corrcoef(class_Parietal_RH(class_Parietal_RH(:,3)==2,1),resid_Parietal_RH(resid_Parietal_RH(:,3)==2,1));

Temporal_Att_LH = corrcoef(class_Temporal_LH(class_Temporal_LH(:,3)==1,1),resid_Temporal_LH(resid_Temporal_LH(:,3)==1,1));
Temporal_Ig_LH = corrcoef(class_Temporal_LH(class_Temporal_LH(:,3)==2,1),resid_Temporal_LH(resid_Temporal_LH(:,3)==2,1));
Temporal_Att_RH = corrcoef(class_Temporal_RH(class_Temporal_RH(:,3)==1,1),resid_Temporal_RH(resid_Temporal_RH(:,3)==1,1));
Temporal_Ig_RH = corrcoef(class_Temporal_RH(class_Temporal_RH(:,3)==2,1),resid_Temporal_RH(resid_Temporal_RH(:,3)==2,1));

Occipital_Att_LH = corrcoef(class_Occipital_LH(class_Occipital_LH(:,3)==1,1),resid_Occipital_LH(resid_Occipital_LH(:,3)==1,1));
Occipital_Ig_LH = corrcoef(class_Occipital_LH(class_Occipital_LH(:,3)==2,1),resid_Occipital_LH(resid_Occipital_LH(:,3)==2,1));
Occipital_Att_RH = corrcoef(class_Occipital_RH(class_Occipital_RH(:,3)==1,1),resid_Occipital_RH(resid_Occipital_RH(:,3)==1,1));
Occipital_Ig_RH = corrcoef(class_Occipital_RH(class_Occipital_RH(:,3)==2,1),resid_Occipital_RH(resid_Occipital_RH(:,3)==2,1));
